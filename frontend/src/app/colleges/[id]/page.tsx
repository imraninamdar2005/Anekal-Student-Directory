...
export default function CollegeDetailsPage() {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        console.error('Failed fetching college drilldown', e);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!data) {
    return (
      <AppShell>
        <div className="p-8 text-center text-slate-500">College record not found.</div>
      </AppShell>
    );
  }

  const { college, total_students, academic_years_breakdown, passout_years_breakdown, students } = data;
  ...
  const exportStudentsToExcel = (studentList: any[], filterLabel: string) => {
    const cleanCollegeName = (college.college_name || 'College').replace(/[^a-zA-Z0-9_-]/g, '_');
    const cleanLabel = filterLabel.replace(/[^a-zA-Z0-9_-]/g, '_');
    const fileName = `${cleanCollegeName}_${cleanLabel}_Students.xlsx`;
    ...
    const exportRows = studentList.map((st: any, idx: number) => ({
      ...
      'Course': st.course_degree || '',
      'Branch': st.branch || '',
      'Year / Semester': st.current_year_sem || '',
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 text-xs text-slate-500 mt-1">
                <span className="flex items-center gap-1">
                  ...
                  {college.locality || 'Anekal'}
                </span>
                {college.created_at && (
                  <span className="flex items-center gap-1 text-[11px] text-slate-400">
                    <Clock className="w-3 h-3" />
                    Added: {formatDate(college.created_at)}
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              ...
              <div className="text-2xl font-bold text-indigo-700">{total_students} Students</div>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Browse and filter enrolled college students for specific academic terms (e.g., 2025-26).
              </p>
            </div>
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-[11px] font-bold text-slate-600 uppercase">
                  ...
                  <th className="py-3 px-4">Current Year/Sem</th>
                  ...
                  section1Students.map((st: any) => (
                    <tr key={st.id} className="hover:bg-slate-50">
                      ...
                      <td className="py-3 px-4 text-slate-700">
                        <span className="font-semibold">{st.course_degree || '—'}</span>
                        {st.branch && <span className="text-slate-500"> ({st.branch})</span>}
                      </td>
                      <td className="py-3 px-4 text-slate-600">{st.current_year_sem || '—'}</td>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden space-y-4 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Filter college students by their anticipated or completed graduation year (e.g., 2026, 2027).
              </p>
            </div>
